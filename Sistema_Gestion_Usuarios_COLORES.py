...
def crear_tabla():
    ...
    for columna in ("correo", "puesto", "observaciones"):
    ...
    try:
        cursor.execute("""
            INSERT INTO usuarios (
                ...
                correo, puesto, observaciones, imagen, archivo
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            nombre.get().strip(),
...
def seleccionar_usuario(event=None):
    ...
    observaciones.delete("1.0", "end")
    observaciones.insert("1.0", valores[11] or "")
...
Label(formulario, text="Estado:").grid(
    row=3, column=2, sticky="e", padx=(18, 8), pady=6
)
